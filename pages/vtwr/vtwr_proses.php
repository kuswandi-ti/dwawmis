<?php
	...
	$barcode_id = $_POST['txtvendtowhsretur'];
	$proses = $_GET['proses'];
	...
	if(!$query_temp) {
		echo "<script>alert('Maaf Proses No Document $doc_number ini, belum terdapat barcode yang terScan.');location.href='vtwr.php?proses=".$proses."'</script>";
	} else {
		...
		/* 1. Insert ke tabel VTW */
		$query_wtv = mssql_query ("INSERT INTO TTrx_Sls_PalletIn_SupplierToWarehouse (Doc_Number,Barcode_Id,SJ_Subcont_Number,Barcode_From,Barcode_To,Scan_Date,Scan_Time,Scan_Flag,Rec_UserId,Rec_LastDate,Rec_LastTime)
            	     SELECT '".$doc_number."',Barcode_Id,'".$_SESSION['sj_number_vendor_retur']."','SUPP','WHS','".$date."','".$date."',0,'".$_SESSION['operator']."','".$date."','".$date."' 
					 FROM TTmp_Sls_ScanBarcodePallet 
					 WHERE Computer_Name='".$_SESSION['computer_name']."' AND Proses='$proses'");
					 
		/* 2. Delete tabel flow */
		$delete_flow = mssql_query("DELETE FROM TTrx_Flow_Pallet_Supplier WHERE Trx_Name='wtv' AND Barcode_Id IN
		...
		$query_flow = mssql_query ("INSERT INTO TTrx_Flow_Pallet_Supplier (Doc_Number,Trx_Name,Barcode_Id) 
		                            SELECT '".$doc_number."','vtw',Barcode_Id 
									FROM TTmp_Sls_ScanBarcodePallet
									WHERE Computer_Name='".$_SESSION['computer_name']."' AND Proses='".$proses."'");
