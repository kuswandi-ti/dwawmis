<?php
	...
	if(!$query_temp) {
		echo "<script>alert('Maaf Proses No Document $doc_number ini, belum terdapat barcode yang terScan.');location.href='cte.php?proses=".$proses."'</script>";
	} else  {
		...
		/* 1. Insert ke tabel CTE */
		$query_cte = mssql_query ("INSERT INTO TTrx_Sls_PalletIn_CustomerToEkspedisi (Doc_Number,Barcode_Id,Barcode_From,Barcode_To,Scan_Date,Scan_Time,Scan_Flag,Rec_UserId,Rec_LastDate,Rec_LastTime)
            	     SELECT '".$doc_number."',Barcode_Id,'CUST','EXP','".$date."','".$date."',1,'".$_SESSION['operator']."','".$date."','".$date."' 
					 FROM TTmp_Sls_ScanBarcodePallet WHERE Computer_Name='".$_SESSION['computer_name']."' AND Proses='".$proses."'");
		
		/* 2. Delete tabel flow */
		$delete_flow = mssql_query("DELETE FROM TTrx_Flow_Pallet_Customer WHERE Trx_Name='stc' AND Barcode_Id IN
