<?php
	...
	if(!$query_temp) {
		echo "<script>alert('Maaf Proses No Document $doc_number ini, belum terdapat barcode yang terScan.');location.href='vtw.php?proses=".$proses."'</script>";
	} else {
		...
		$query_vtw = mssql_query ("INSERT INTO TTrx_Sls_PalletIn_SupplierToWarehouse (Doc_Number,Barcode_Id,Barcode_From,Barcode_To,Scan_Date,Scan_Time,Scan_Flag,Rec_UserId,Rec_LastDate,Rec_LastTime)
            	     SELECT '".$doc_number."',Barcode_Id,'SUPP','WHS','".$date."','".$date."',1,'".$_SESSION['operator']."','".$date."','".$date."' 
		...
		$query_flow = mssql_query ("INSERT INTO TTrx_Flow_Pallet_Supplier (Doc_Number,Trx_Name,Barcode_Id) 
		                            SELECT '".$doc_number."',Proses,Barcode_Id 
									FROM TTmp_Sls_ScanBarcodePallet
									WHERE Computer_Name='".$_SESSION['computer_name']."' AND Proses='".$proses."'");
