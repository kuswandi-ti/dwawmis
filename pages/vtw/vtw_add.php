<?php
	...
	$query_temp = mssql_fetch_array(mssql_query("SELECT * FROM TTmp_Sls_ScanBarcodePallet 
									...
									AND Proses='".$proses."'"));
	$query_wtv = mssql_fetch_array(mssql_query("SELECT * FROM TTrx_Sls_PalletOut_WarehouseToSupplier 
									WHERE Barcode_Id='".$barcode_id."' AND Barcode_To='SUPP' AND Scan_Flag=0")); 
	$cek_flow = mssql_fetch_array(mssql_query("SELECT * FROM TTrx_Flow_Pallet_Supplier WHERE Barcode_Id='".$barcode_id."' AND Trx_Name='wtv'"));
	$cek_trx = mssql_fetch_array(mssql_query("SELECT * FROM QView_Sls_Trx_ScanBarcodePallet_VTW WHERE Barcode_Id='".$barcode_id."'"));
	...
	/* 5. Cek, apakah barcode sudah pernah discan */
		...
		echo "<script>alert('Pallet masih ada di Store (Transaksi VTW). No. Transaksi $doc_number_trx tanggal $doc_date_trx_format');
		     location.href='$proses.php?proses=".$proses."'</script>";
	}
	
	/* 6. Cek, apakah barcode masih di DWA atau di VENDOR */
    else if (!$cek_flow) {
		echo "<script>alert('Posisi pallet ada di DWA. Proses transaksi keluar dulu!');
		     location.href='$proses.php?proses=".$proses."'</script>";
	}
	...
	/* 7. Cek, apakah barcode sudah discan di transaksi ini */
		...
		$_SESSION['doc_num_wtv_source'] = ($query_wtv['Doc_Number']);
