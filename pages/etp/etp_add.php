<?php
	...
	$query_temp = mssql_fetch_array(mssql_query("SELECT * FROM TTmp_Sls_ScanBarcodePallet 
									...
									AND Proses='".$proses."'"));
	$cek_flow = mssql_fetch_array(mssql_query("SELECT * FROM TTrx_Flow_Pallet_Customer WHERE Barcode_Id='".$barcode_id."' AND Trx_Name='cte'"));
	$cek_trx = mssql_fetch_array(mssql_query("SELECT * FROM QView_Sls_Trx_ScanBarcodePallet_ETP WHERE Barcode_Id='".$barcode_id."'"));
	...
	/* 5. Cek, apakah barcode sudah pernah discan */
		...
		echo "<script>alert('Pallet masih ada di Produksi (Transaksi ETP). No. Transaksi $doc_number_trx tanggal $doc_date_trx_format');
		     location.href='$proses.php?proses=".$proses."'</script>";
	}
	
	/* 6. Cek, apakah barcode masih di STORE atau tidak */
    else if (!$cek_flow) {
		echo "<script>alert('Posisi pallet tidak ada di STORE!');
		     location.href='$proses.php?proses=".$proses."'</script>";
	}	
